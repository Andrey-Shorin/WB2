...
var (
	...
	ErrNotSetFields    = errors.New("вы не выбрали колонки, которые необходимо вывести. флаг -f")
	ErrIncorrectFields = errors.New("не корректно указаны параметры флага -f, корректно будет указать: '-f 1' '-f 1,2,3' '-f 1-5'")
	ErrOutOfRange      = errors.New("колонки с таким номером(ами) не существует, попробуйте поменять значение флага -f")
)
